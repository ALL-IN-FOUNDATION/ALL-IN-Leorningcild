...
nav{position:fixed;top:var(--ticker-h);left:0;right:0;z-index:599;
  ...
  backdrop-filter:blur(24px);box-sizing:border-box}
...
.wbar{border:1px solid var(--border);
  ...
  flex-wrap:wrap;margin-bottom:1.1rem;position:relative;overflow:hidden}
...
.pitem.running::before{content:'';position:absolute;left:0;top:0;bottom:0;
  width:2px;background:var(--green2);opacity:.6}
...
.pnl-cell::before{content:'';position:absolute;top:0;left:25%;right:25%;height:1px;
  background:linear-gradient(90deg,transparent,rgba(212,168,83,.08),transparent)}
...
.mbtn-p{flex:1;font-family:'Barlow Condensed',sans-serif;font-weight:700;
  ...
  touch-action:manipulation;-webkit-tap-highlight-color:rgba(192,57,43,.3)}
...
.lock{position:absolute;inset:0;z-index:10;background:rgba(1,3,10,.65);
  backdrop-filter:blur(3px);display:flex;align-items:center;justify-content:center;
  flex-direction:column;gap:.65rem;text-align:center;padding:1rem}
...
var WORKER_URL = (function(){
  ...
  return 'https://leorningcild.all-in-foundation-project.workers.dev';
})();
...
async function workerFetch(path, method, body){
  ...
  var opts = {
    method  : method,
    headers : { 'Content-Type': 'application/json' },
    signal  : (function(){ var c=new AbortController(); setTimeout(function(){c.abort();},12000); return c.signal; })()
  };
  ...
  for(var i = 0; i < rpcs.length; i++){
    try{
      var r = await fetch(rpcs[i], {
        ...
        signal  : (function(){ var c=new AbortController(); setTimeout(function(){c.abort();},6000); return c.signal; })()
      });
      var d = await r.json();
...
async function fetchTokenBalances(addr){
  ...
  // 3. Fetch prices from Jupiter for all mints at once
  ...
  // Get symbol from Jupiter token list if missing
    ...
    for(var si = 0; si < Math.min(rawTokens.length, 20); si++){
      ...
      try{
        var sr = await fetch(RPC_LIST[0], {
          ...
          signal: (function(){ var c=new AbortController(); setTimeout(function(){c.abort();},4000); return c.signal; })()
        });
  ...
  // Render table
  var thSt = 'font-family:Barlow Condensed,sans-serif;font-size:.42rem;letter-spacing:.16em;color:rgba(212,168,83,.5);text-transform:uppercase;padding:.3rem .4rem;font-weight:700';
  var moSt = 'font-family:Share Tech Mono,monospace;';
  var h = '<table style="width:100%;border-collapse:collapse">';
  h += '<tr style="border-bottom:1px solid rgba(212,168,83,.08)">';
  h += '<th style="' + thSt + ';text-align:left">TOKEN</th>';
  h += '<th style="' + thSt + ';text-align:right">AMOUNT</th>';
  h += '<th style="' + thSt + ';text-align:right">% SUPPLY</th>';
  h += '<th style="' + thSt + ';text-align:right">USD</th>';
  h += '</tr>';
  ...
  rawTokens.forEach(function(t){
    ...
    h += '<div style="' + moSt + 'font-size:.54rem;color:var(--cream);font-weight:600">' + esc(t.sym) + '</div>';
    h += '<div style="' + moSt + 'font-size:.38rem;color:rgba(237,232,220,.18);margin-top:.05rem">' + t.mint.slice(0,8) + '…</div>';
    h += '</td>';
    // Amount
    h += '<td style="padding:.38rem .4rem;text-align:right">';
    h += '<div style="' + moSt + 'font-size:.52rem;color:var(--gold-b)">';
    ...
    h += '</div>';
    if(t.price > 0) h += '<div style="' + moSt + 'font-size:.38rem;color:rgba(237,232,220,.22)">$'+fmtP(t.price)+'/ea</div>';
    h += '</td>';
    // % Supply
    h += '<td style="padding:.38rem .4rem;text-align:right">';
    if(t.pct !== null){
      ...
      h += '<div style="' + moSt + 'font-size:.52rem;color:'+pctColor+'">'+pctStr+'</div>';
    } else {
      h += '<div style="' + moSt + 'font-size:.52rem;color:rgba(237,232,220,.18)">—</div>';
    }
    h += '</td>';
    // USD
    h += '<td style="padding:.38rem .4rem;text-align:right">';
    if(t.usd > 0){
      h += '<div style="' + moSt + 'font-size:.56rem;color:#3ddc6a;font-weight:600">$'+t.usd.toFixed(2)+'</div>';
    } else {
      h += '<div style="' + moSt + 'font-size:.52rem;color:rgba(237,232,220,.18)">—</div>';
    }
    h += '</td>';
...
// Check for Phantom Connect API callback on page load
function checkPhantomCallback(){
  ...
  if(urlParams.get('phantom_connect') === '1'){
    ...
    if(pubKey && data){
      // Phantom connected — get public key from data
      // Data contains encrypted { public_key, session }
      // Fallback: trigger prov.connect() as session should exist
      setTimeout(function(){
        var prov = getProvider();
...
function walletClick(){
  ...
  // Re-check provider each time — Phantom may inject after page load
  var prov = getProvider();
  if(prov){
...
function openPhModal(){
  ...
  if(ENV.mob){
    html = '<img src="'+logo+'" style="width:48px;height:48px;border-radius:12px;margin:0 auto .75rem;display:block" alt="Phantom">' +
      '<div class="mtext"><b>Choose how to connect:</b></div>' +
      '<div class="ph-opts">' +
        '<button class="ph-opt primary" onclick="closePhModal();phantomConnectRedirect()" style="flex-direction:column;gap:.25rem;padding:.85rem">' +
          '<span style="display:flex;align-items:center;gap:.5rem"><img src="'+logo+'" style="width:18px;height:18px;border-radius:4px"> Connect & Return to Browser</span>' +
          '<span style="font-size:.44rem;opacity:.65;font-weight:400;letter-spacing:.04em">Opens Phantom to approve, then redirects back here</span>' +
        '</button>' +
        '<div class="ph-div">or</div>' +
        '<a href="'+ul+'" class="ph-opt" style="flex-direction:column;gap:.2rem;padding:.75rem">' +
          '<span>Open in Phantom Browser</span>' +
          '<span style="font-size:.42rem;opacity:.6;font-weight:400">Page opens inside Phantom built-in browser</span>' +
        '</a>' +
        '<div class="ph-div">don\'t have Phantom?</div>' +
        (ENV.andr
      ...
      '</div>' +
      '<div class="ph-note">Option 1: Approve in Phantom then auto-redirect back to Chrome.<br>Option 2: Page stays open inside Phantom browser.</div>';
  } else {
...
window.addEventListener('load', function(){
  ...
  // Check Phantom Connect API callback on load
  checkPhantomCallback();

  // Measure actual nav height dynamically
  (function fixLayout(){
...
async function handlePrice(url) {
  ...
  if (!mints) return jsonResp({ ok: false, error: 'Missing mints param' }, 400);
  try {
    const r = await fetch(`${JUP_PRICE}?ids=${mints}`);
    const d = await r.json();
    return jsonResp({ ok: true, data: d.data || {} });
  } catch (e) { return jsonResp({ ok: false, error: e.message, data: {} }); }
}
...
async function handleQuote(url) {
  try {
    ...
    const d = await r.json();
    if (d.error || !d.outAmount) return jsonResp({ ok: false, error: d.error || 'No route found', quote: null });
    return jsonResp({ ok: true, quote: d });
  } catch (e) { return jsonResp({ ok: false, error: e.message, quote: null }); }
}
...
async function handleSwapTx(request) {
  try {
    ...
    if (!body.quoteResponse || !body.userPublicKey) {
      return jsonResp({ ok: false, error: 'Missing quoteResponse or userPublicKey' }, 400);
    }
    ...
    const d = await r.json();
    if (!d.swapTransaction) return jsonResp({ ok: false, error: d.error || 'No swap transaction returned' });
...
async function handleTokenInfo(url) {
  ...
  if (!mint) return jsonResp({ ok: false, error: 'Missing mint param' }, 400);
  try {
