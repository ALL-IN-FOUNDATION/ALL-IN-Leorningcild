...
/* ─ NAV ─ */
nav{position:fixed;top:28px;left:0;right:0;z-index:599;display:flex;align-items:center;justify-content:space-between;padding:.7rem 1rem;background:rgba(2,4,10,.95);border-bottom:1px solid rgba(212,168,83,.1);backdrop-filter:blur(20px);overflow:hidden;box-sizing:border-box}
.nav-logo{font-family:'Libre Baskerville',serif;font-style:italic;font-weight:700;font-size:1rem;letter-spacing:.04em;color:var(--gold);display:flex;align-items:center;gap:.35rem;text-decoration:none;min-width:0;overflow:hidden;flex-shrink:1}
.nav-badge{font-family:'Barlow Condensed',sans-serif;font-style:normal;font-weight:700;font-size:.42rem;letter-spacing:.14em;text-transform:uppercase;padding:.18rem .45rem;border:1px solid rgba(212,168,83,.22);color:var(--gold);background:rgba(212,168,83,.05);white-space:nowrap;flex-shrink:0}
.ph-btn{display:inline-flex;align-items:center;gap:.35rem;font-family:'Barlow Condensed',sans-serif;font-weight:700;font-size:.62rem;letter-spacing:.1em;text-transform:uppercase;padding:.42rem .75rem;background:transparent;color:var(--cream);border:1px solid rgba(171,159,242,.35);cursor:pointer;transition:all .22s;flex-shrink:0;white-space:nowrap}
...
/* ─ WORKER STATUS BAR ─ */
.worker-bar{background:rgba(6,13,20,.9);border-bottom:1px solid rgba(212,168,83,.05);padding:.28rem 1.5rem;display:flex;align-items:center;gap:1rem;flex-wrap:wrap;position:fixed;top:calc(28px + 52px);left:0;right:0;z-index:598}
.wb-item{display:flex;align-items:center;gap:.35rem;font-family:'Share Tech Mono',monospace;font-size:.46rem;letter-spacing:.08em;color:rgba(237,232,220,.28)}
...
/* ─ PAGE HEADER ─ */
.page-header{padding:calc(28px + 52px + 24px + 2.2rem) 1.5rem 1.6rem;max-width:1200px;margin:0 auto;overflow:hidden;box-sizing:border-box;width:100%}
...
.eyebrow-txt{font-size:.5rem;font-weight:700;letter-spacing:.18em;color:var(--teal);text-transform:uppercase;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.page-title{font-family:'Libre Baskerville',serif;font-weight:700;font-style:italic;font-size:clamp(1rem,5vw,3.2rem);color:var(--cream);line-height:1.1;margin-bottom:.65rem;word-break:break-word;overflow-wrap:anywhere;max-width:100%}
...
@media(max-width:480px){
  ...
  .nav-logo{font-size:.88rem;gap:.28rem}
  .nav-badge{font-size:.38rem;letter-spacing:.1em;padding:.14rem .35rem}
  .ph-btn{font-size:.56rem;letter-spacing:.07em;padding:.38rem .6rem;gap:.28rem}
  .ph-btn img{width:16px;height:16px}
  .eyebrow-txt{letter-spacing:.12em;font-size:.46rem}
  .page-title{font-size:clamp(.95rem,4.8vw,2rem)}
  .worker-bar{padding:.28rem .75rem;gap:.5rem}
  .wb-item{font-size:.42rem}
  .page-header{padding:calc(28px + 48px + 24px + 1.8rem) .75rem 1.2rem}
}
...
.bal-tokens{padding:.55rem 1rem;border-top:1px solid rgba(212,168,83,.06);max-height:160px;overflow-y:auto;scrollbar-width:thin;scrollbar-color:rgba(212,168,83,.1) transparent}
...
async function executeSell(pos, confirmBtn){
  ...
  var decimals  = S.tokenCache[tokenMint]?.decimals || 6;
...
async function fetchTokenBalances(addr){
  ...
  for(var i = 0; i < rpcs.length; i++){
    try{
      ...
      var d = await r.json();
      if(!d.result || !d.result.value) continue;
      var tokens = [];
      var totalUsd = S.balance * (S.solPrice || 0);
      d.result.value.forEach(function(acc){
        try{
          var info = acc.account.data.parsed.info;
          var amt  = parseFloat(info.tokenAmount.uiAmount || 0);
          if(amt < 0.000001) return; // skip dust
          var mint = info.mint;
          var sym  = S.tokenCache[mint] ? S.tokenCache[mint].symbol : mint.slice(0,6)+'…';
          var price = 0;
          // Check if we have price from positions
          S.positions.forEach(function(p){ if(p.mint===mint) price = p.curPrice; });
          var usd = price > 0 ? amt * price : null;
          if(usd) totalUsd += usd;
          tokens.push({ sym, amt, mint, usd });
        }catch(e){}
      });
      // Update UI
      document.getElementById('balTokCount').textContent = tokens.length;
      if(tokens.length === 0){
        document.getElementById('balTokenList').innerHTML = '<div class="bal-empty">No SPL tokens found</div>';
      } else {
        var h = '';
        tokens.sort(function(a,b){ return (b.usd||0)-(a.usd||0); }).forEach(function(t){
          h += '<div class="bal-tok-row">';
          h += '<span class="bal-tok-name">'+esc(t.sym)+'</span>';
          h += '<div style="text-align:right">';
          h += '<div class="bal-tok-amt">'+t.amt.toLocaleString(undefined,{maximumFractionDigits:4})+'</div>';
          if(t.usd != null) h += '<div class="bal-tok-usd">$'+t.usd.toFixed(2)+'</div>';
          h += '</div></div>';
        });
        document.getElementById('balTokenList').innerHTML = h;
      }
      // Update total USD including tokens
      if(S.solPrice > 0){
        document.getElementById('balUSD').textContent = '$' + totalUsd.toFixed(2);
      }
      return;
    }catch(e){ console.warn('fetchTokenBalances error:', e); }
  }
}
...
function connectLink(){
  // Phantom Connect API — connect wallet then redirect back to this page (Chrome stays open)
  // This is the correct flow for connecting FROM Chrome, not opening in Phantom browser
  var params = new URLSearchParams({
    app_url    : location.origin,
    dapp_encryption_public_key: '',  // placeholder, not needed for basic redirect
    redirect_link: location.href,
    cluster    : 'mainnet-beta'
  });
  // Use the simpler solana: URI scheme for just opening Phantom
  return 'https://phantom.app/ul/v1/connect?' + params.toString();
}

function walletClick(){
  if(S.connected){ doDisconnect(); return; }
  // Re-check provider in case Phantom injected after page load
  var prov = getProvider();
  if(prov){ ENV.inPh = true; ENV.ext = true; doConnect(); return; }
  if(ENV.inPh || ENV.ext){ doConnect(); return; }
  openPhModal();
}
...
function openPhModal(){
  ...
  if(ENV.mob){
    html = '<img src="'+logo+'" style="width:52px;height:52px;border-radius:14px;margin:0 auto .8rem;display:block" alt="Phantom">' +
      '<div class="mtext">Pilih cara connect <b style="color:var(--phantom)">Phantom Wallet</b>:</div>' +
      '<div class="ph-opts">' +
        '<a href="'+ul+'" class="ph-opt primary" onclick="closePhModal();addLog(\'Opening Phantom...\',\'inf\')" ><img src="'+logo+'"> Buka di Browser Phantom</a>' +
        '<div class="ph-div">atau pakai deep link</div>' +
        '<a href="'+dl+'" class="ph-opt" onclick="closePhModal()">Open via Deep Link</a>' +
        '<div class="ph-div">belum punya Phantom?</div>' +
        (ENV.andr ? '<a href="https://play.google.com/store/apps/details?id=app.phantom" target="_blank" class="ph-opt" onclick="closePhModal()">Install — Google Play</a>'
                  : '<a href="https://apps.apple.com/app/phantom-solana-wallet/id1598432977" target="_blank" class="ph-opt" onclick="closePhModal()">Install — App Store</a>') +
      '</div>' +
      '<div class="ph-note">Setelah di Phantom, kembali ke browser ini lalu tap Connect Wallet lagi — wallet akan terdeteksi otomatis.</div>';
  } else {
    html = '<img src="'+logo+'" style="width:52px;height:52px;border-radius:14px;margin:0 auto .8rem;display:block" alt="Phantom">' +
